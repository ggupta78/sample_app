...
class UsersIndexAdminTest < UsersIndexAdmin
  ...
  test 'should display only activated users' do
    ...
    # Making an inactive fixture user isn't sufficient because
    # Rails can't guarantee it would appear on the first page.
    User.paginate(page: 1).first.toggle!(:activated)
    # Ensure that all displayed users are activated
